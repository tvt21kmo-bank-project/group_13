...
BEGIN
    ...
    IF tili=0 THEN
		CALL lisaa_tili(asiakas, saldo, luottoraja);
	ELSE
...
-- -----------------------------------------------------
-- procedure lisaa_tapahtuma
-- -----------------------------------------------------

DELIMITER $$
USE `bank_simul`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `lisaa_tapahtuma`(
IN tyyppi INT,
IN summa INT,
IN tili INT,
IN kortti INT)
BEGIN
	...
	IF tyyppi=0 OR tyyppi = 2 THEN
		UPDATE accounts SET balance=balance-summa WHERE idaccount=tili;
        SELECT balance INTO saldo FROM accounts WHERE idaccount=tili;
	ELSEIF tyyppi=1 OR tyyppi=3 THEN
		UPDATE credit SET balance=balance+summa WHERE idaccount=tili;
        SELECT balance INTO saldo FROM credit WHERE idaccount=tili;
	ELSEIF tyyppi=4 THEN
		UPDATE accounts SET balance=balance+summa WHERE idaccount=tili;
        SELECT balance INTO saldo FROM accounts WHERE idaccount=tili;
	ELSEIF tyyppi=5 THEN
		UPDATE credit SET balance=balance-summa WHERE idaccount=tili;
        SELECT balance INTO saldo FROM credit WHERE idaccount=tili;
	END IF;
	INSERT INTO transactions (transaction_type, transaction_amount, idaccount, idcard, balance) 
		VALUES(tyyppi, summa, tili, kortti, saldo);
END$$
...
BEGIN
	...
	ELSE
		INSERT INTO accounts (idOwner, balance) values(asiakasID, saldo);
        SELECT LAST_INSERT_ID() INTO tili;
        INSERT INTO accounts_has_customers (idAccount,idCustomer) VALUES (tili,asiakasID);
        INSERT INTO credit (idaccount, credit_limit, balance) VALUES (tili, luottoraja, 0);
        INSERT INTO cards (pin, failed_attempts, has_credit, idaccount, idcustomer) VALUES (1234, 0, 0, tili, asiakasID);
        IF luottoraja > 0 THEN
			INSERT INTO cards (pin, failed_attempts, has_credit, idaccount, idcustomer) VALUES (1234, 0, 1, tili, asiakasID);
        END IF;
...
BEGIN
	...
	IF luotto=false THEN
        ...
        IF summa <= saldo THEN
			CALL lisaa_tapahtuma(0, summa, tiliID, korttiID);
            SELECT 'OK';
        ELSE
			SELECT 'FAIL';
        END IF;
	ELSE
        ...
        IF summa+saldo <= luottoraja THEN
			CALL lisaa_tapahtuma(1, summa, tiliID, korttiID);
			SELECT 'OK';
        ELSE
			SELECT 'FAIL';
        END IF;        
...
BEGIN
	DECLARE tili INT DEFAULT 0;
    DECLARE luotto INT DEFAULT 0;
    SELECT idaccount INTO tili FROM accounts WHERE idOwner=asiakasID;
    SELECT idcredit INTO luotto FROM credit WHERE idaccount=tili;
    DELETE FROM credit WHERE idcredit=luotto;
...
BEGIN
	SELECT * FROM transactions WHERE idaccount=tiliID ORDER BY idtransaction DESC LIMIT alkaen_tapahtumasta,10;
END$$
